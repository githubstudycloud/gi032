...
<template>
  ...
  >
    <div class="flex items-start justify-between gap-2 min-h-[34px]">
      <span class="text-[12px] text-ink-600 leading-snug line-clamp-2">{{ metric.label }}</span>
      <span
    ...
    <div class="mt-2 font-display text-[24px] font-semibold text-ink-900 leading-none tabular-nums">
      {{ metric.value }}<span v-if="metric.unit" class="ml-1 text-[13px] font-normal text-ink-500">{{ metric.unit }}</span>
    </div>
    <div
      :class="[
